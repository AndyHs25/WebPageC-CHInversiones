// Carrusel de Marcas - Funcionalidad Simplificada
let currentBrandSlide = 0;
const totalBrandSlides = 8; // 12 marcas, mostrando 4-5 a la vez, máximo 8 posiciones
...
function controlBrandCarousel(direction) {
    console.log('Control carousel:', direction);
    stopBrandCarousel();
...
function goToBrandSlide(groupIndex) {
    console.log('Go to slide:', groupIndex);
    stopBrandCarousel();
...
// Función para obtener el ancho real del elemento
function getItemWidth() {
    const isMobile = window.innerWidth <= 768;
    if (isMobile) {
        return 170; // 150px width + 20px gap
    } else {
        return 230; // 200px width + 30px gap
    }
}
...
function updateBrandCarousel() {
    ...
    // Usar ancho dinámico basado en el dispositivo
    ...
    console.log(`Slide actual: ${currentBrandSlide}, translateX: ${translateX}px, itemWidth: ${itemWidth}px`);
    
    // Actualizar indicadores basado en grupos de 3 marcas
    const currentGroup = Math.min(Math.floor(currentBrandSlide / 3), 3);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (brandsSection) {
        ...
        console.log('Event listeners de hover agregados');
    }
...
function optimizeForMobile() {
    ...
    if (isMobile) {
        ...
        // Actualizar carrusel con nuevas medidas
        updateBrandCarousel();
    }
}
...
window.controlBrandCarousel = controlBrandCarousel;
window.goToBrandSlide = goToBrandSlide;
